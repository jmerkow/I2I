...
class Net {
 public:
  ...
  void set_gpu_memory_info(const bool value) { gpu_memory_info_ = value; }
 ...
 protected:
  ...
  /// @brief Helper for displaying gpu memory info.
  void inline GPUMemoryInfo(const int layer_id, const string& state){
    if (layer_id < 0) {
      LOG(INFO) << "    [" << state << "] "<< gpu_memory_string(); 
    } else {
      LOG(INFO) << "    [" << state << "] "
       << "Layer " << layer_names_[layer_id] << ", " << gpu_memory_string(); 
    }
  }
  ...
  bool debug_info_;
  /// Whether to compute and display gpu memory info for the net.
  bool gpu_memory_info_;
