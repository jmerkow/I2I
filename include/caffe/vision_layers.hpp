...
class BaseConvolutionLayer : public Layer<Dtype> {
 public:
  ...
  virtual inline int channel_axis(){return channel_axis_;}
  virtual inline int channels(){return channels_;}
 ...
 protected:
  ...
  int num_shared_buffers_;
  vector< Blob<Dtype> > shared_buffers_;
  vector< vector<int> > shared_buffer_shapes_;
...
class ConvolutionLayer : public BaseConvolutionLayer<Dtype> {
 public:
  ...
  virtual inline DiagonalAffineMap<Dtype> coord_map(int num_spatial_axes) {
  CHECK_EQ(this->num_spatial_axes_, num_spatial_axes);
  const int* kernel_shape_data = this->kernel_shape_.cpu_data();
  const int* stride_data = this->stride_.cpu_data();
  const int* pad_data = this->pad_.cpu_data();
    return FilterMap<Dtype>(this->num_spatial_axes_,kernel_shape_data,
      stride_data,pad_data).inv();
  }
...
class LRNLayer : public Layer<Dtype> {
 public:
  ...
  virtual inline int ExactNumTopBlobs() const { return 1; }
  virtual inline DiagonalAffineMap<Dtype> coord_map(int num_spatial_axes) {
    return DiagonalAffineMap<Dtype>::identity(num_spatial_axes);
  }
...
class CropLayer : public Layer<Dtype> {
 public:
  ...
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
//  virtual inline Caffe::Brew ModeOverride() const { return Caffe::CPU; }
  ...
  virtual inline DiagonalAffineMap<Dtype> coord_map(int num_spatial_axes) {
    ...
    for (int i = 0;i < this->num_spatial_axes_;++i) {
      coefs.push_back(make_pair(1, - crop_data[i]));
    }
