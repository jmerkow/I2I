...
def get_layer_label(layer, rankdir,**kwargs):
    ...
    if layer.type == 'Convolution':
        ...
        node_label = '"%s%s(%s %s)%skernel size: %d%sstride: %d%spad: %d"' %\
                     (layer.name,
                      ...
                      layer.type,
                      separator,
                      layer.pooling_param.kernel_size[0] if len(param.kernel_size._values) else 1,
                      separator,
                      layer.pooling_param.stride[0] if len(param.stride._values) else 1,
                      separator,
                      layer.pooling_param.pad[0] if len(param.pad._values) else 0)
    else:
...
def get_pydot_graph(caffe_net, rankdir, label_edges=True,**kwargs):
    ...
    for layer in caffe_net.layer:
        node_label = get_layer_label(layer, rankdir, **kwargs)
...
def draw_net(caffe_net, rankdir, ext='png',**kwargs):
